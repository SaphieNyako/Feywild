...
public class ModEventListener {
    ...
    public void onPlayerTick(PlayerTickEvent.Post event) {
        ...
        boolean hasFeyFlying = player.hasEffect(ModEffects.FEY_FLYING);

        CompoundTag data = player.getPersistentData();

        boolean grantedByFeywild = data.getBoolean(FEYWILD_GRANTED_FLIGHT);

        var abilities = player.getAbilities();

        if (!hasFeyFlying) {
            // Do not remove flight granted by another mod.
            if (grantedByFeywild) {
        ...
        MobEffectInstance effect = player.getEffect(ModEffects.FEY_FLYING);
