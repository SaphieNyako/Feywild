...
public abstract class TreeEntBase extends FeyBase implements GroundEntity, PlayerRideableJumping {
    ...
    public boolean isDamageSourceBlocked(DamageSource damageSource) {
        if (this.isBlocking() && !damageSource.isBypassArmor()) {
            ...
            if (sourcePos != null) {
                Vec3 lookVec = this.calculateViewVector(0.0F, this.getYHeadRot());
                Vec3 attackVec = sourcePos.vectorTo(this.position());
                attackVec = new Vec3(attackVec.x, 0.0, attackVec.z).normalize();
                return attackVec.dot(lookVec) < 0.0;
            }
        }
        return false;
    }
