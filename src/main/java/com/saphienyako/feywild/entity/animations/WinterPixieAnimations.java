...
public class WinterPixieAnimations implements AnimationsInterface{

    // POSE
    public static final RotationKeyframe[] BODY_POSE_ROT = {
    ...
    public static final RotationKeyframe[] HAIR_IDLE_ROT = {
            new RotationKeyframe(0f, 0f, 0f, 0f),
            new RotationKeyframe(2f, -5f, 0f, 0f),
            new RotationKeyframe(4f, 0f, 0f, 0f)
    };
    ...
    public static final RotationKeyframe[] HAIR_FLY_ROT = {
    ...
    // SPELL_CASTING
    public static final RotationKeyframe[] BODY_SPELL_ROT = {
            new RotationKeyframe(0f, -2.5f, 0f, 0f),
    ...
    public static final RotationKeyframe[] HAIR_SPELL_ROT = {
            new RotationKeyframe(0f, 0f, 0f, 0f),
    ...
    public static float[] interpolateRotation(WinterPixieAnimations.RotationKeyframe[] frames, float time) {
        if (frames.length == 0) return new float[]{0f,0f,0f};
        for (int i = 0; i < frames.length - 1; i++) {
            WinterPixieAnimations.RotationKeyframe a = frames[i], b = frames[i+1];
        ...
        WinterPixieAnimations.RotationKeyframe last = frames[frames.length - 1];
        return new float[]{last.xRot,last.yRot,last.zRot};
    }

    public static float[] interpolatePosition(WinterPixieAnimations.PositionKeyframe[] frames, float time) {
        if (frames.length == 0) return new float[]{0f,0f,0f};
        for (int i = 0; i < frames.length - 1; i++) {
            WinterPixieAnimations.PositionKeyframe a = frames[i], b = frames[i+1];
            if (time >= a.time && time <= b.time) {
                float t = (time - a.time) / (b.time - a.time);
                return new float[]{
                        a.x + (b.x - a.x) * t,
        ...
        WinterPixieAnimations.PositionKeyframe last = frames[frames.length - 1];
        return new float[]{last.x,last.y,last.z};
    }

    public static float[] interpolateScale(WinterPixieAnimations.ScaleKeyframe[] frames, float time) {
        if (frames.length == 0) return new float[]{1f,1f,1f};
        for (int i = 0; i < frames.length - 1; i++) {
            WinterPixieAnimations.ScaleKeyframe a = frames[i], b = frames[i+1];
            if (time >= a.time && time <= b.time) {
                float t = (time - a.time) / (b.time - a.time);
                return new float[]{
                        a.x + (b.x - a.x) * t,
        ...
        WinterPixieAnimations.ScaleKeyframe last = frames[frames.length - 1];
        return new float[]{last.x,last.y,last.z};
    }
}
