...
public class SpriteEntity extends FlyingFeyBase {
    ...
    private void onImpact() {
        ...
        switch (variant) {
            ...
            case SUMMER -> {
                level.explode(this, pos.x, pos.y, pos.z, 2.0F, Explosion.BlockInteraction.NONE);
                applyAoE(entity -> {
    ...
    private void setupAnimationStates() {
        ...
        if (getState() != State.HAPPY && getState() != State.ANGRY ) {
            if (isActuallyMoving()) {
        ...
        // ANGRY
        if (getState() == State.ANGRY && getState() != State.HAPPY) {
    ...
    public SpriteVariant getVariant() {
        return SpriteVariant.values()[this.entityData.get(VARIANT)];
    }

    public void setVariant(SpriteVariant variant) {this.entityData.set(VARIANT, variant.ordinal());}
