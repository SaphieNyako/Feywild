...
public class OberonChargingGoal extends Goal {
    ...
    private void damageEntitiesDuringCharge() {
        ...
        for (LivingEntity target : targets) {
            boolean damaged = target.hurt(DamageSource.mobAttack(entity), CHARGE_DAMAGE);
            if (!damaged) {
                continue;
            }
    ...
    private void spawnTeleportPoof(ServerLevel serverLevel, double x, double y, double z) {
        ...
        FeywildNetwork.sendToTrackingEntity(new ParticleMessage(ParticleMessage.Type.DANDELION_FLUFF, position), entity);
