...
public class WinterLeavesBlock extends FeyLeavesBlock {
    ...
    public void animateTick(@Nonnull BlockState state, @Nonnull World world, @Nonnull BlockPos pos, @Nonnull Random rand) {
        ...
        if (world.isEmptyBlock(pos.below()) && rand.nextInt(15) == 1 && ClientConfig.tree_particles) {
            world.addParticle(ModParticles.winterLeafParticle, pos.getX() + rand.nextDouble(), pos.getY(),pos.getZ()+ rand.nextDouble(), 1, -0.1, 0 );
        }
    }
}
