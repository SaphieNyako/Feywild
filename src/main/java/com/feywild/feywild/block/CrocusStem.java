...
public class CrocusStem extends Block {
    ...
    @Override
    public void onRemove(@Nonnull BlockState state, @Nonnull World world, @Nonnull BlockPos pos, @Nonnull BlockState blockState, boolean bool) {
        super.onRemove(state, world, pos, blockState, bool);
        if (world.isClientSide) return;

        if (world.getBlockState(pos.above()).is(ModBlocks.CROCUS_STEM.get())) {
            world.destroyBlock(pos.above(), false);
        }

        if (world.getBlockState(pos.below()).is(ModBlocks.CROCUS_STEM.get())) {
            world.destroyBlock(pos.below(), false);
        }

        if (world.getBlockState(pos.above()).is(ModBlocks.CROCUS.get())) {
