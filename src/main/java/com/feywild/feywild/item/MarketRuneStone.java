...
public class MarketRuneStone extends TooltipItem {
    ...
    public InteractionResultHolder<ItemStack> use(@Nonnull Level level, @Nonnull Player player, @Nonnull InteractionHand hand) {
        if (level.isClientSide) {
            if (level.dimension() == Level.OVERWORLD || level.dimension() == MarketPlaceDimensions.MARKET_PLACE_DIMENSION) {
                return InteractionResultHolder.sidedSuccess(player.getItemInHand(hand), true);
            } else {
                return InteractionResultHolder.fail(player.getItemInHand(hand));
            }
        } else if (player instanceof ServerPlayer) {
            if (level.dimension() == MarketPlaceDimensions.MARKET_PLACE_DIMENSION) {
